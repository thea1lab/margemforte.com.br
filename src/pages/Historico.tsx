...
const Historico = () => {
  ...
  const loadData = useCallback(() => {
    const { data, total: t } = listCalculations({
      page,
      pageSize,
  ...
  const handleImportJSON = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        const count = importCalculations(reader.result as string);
        if (count === 0) {
          toast.warning("Nenhuma simulação nova encontrada no arquivo.");
        } else {
          toast.success(`${count} simulação(ões) importada(s) com sucesso!`);
          loadData();
        }
      } catch {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-secondary/50 to-background">
      <Helmet>
        ...
        <meta
          name="description"
          content="Veja e compare suas simulações de margem de lucro salvas. Exporte para CSV ou JSON, copie resumos e gerencie o histórico de precificação do seu negócio."
        />
        <link rel="canonical" href="https://margemforte.com.br/historico" />
        <meta
          property="og:title"
          content="Histórico de Simulações · Margem Forte"
        />
        <meta
          property="og:description"
          content="Compare simulações de margem de lucro salvas e exporte para CSV ou JSON."
        />
        <meta
          property="og:url"
          content="https://margemforte.com.br/historico"
        />
      </Helmet>
      <Header />
                ...
                <Table className="min-w-[640px] sm:min-w-[720px] md:min-w-0 md:w-full md:table-auto">
                  <TableHeader>
                    <TableRow className="bg-secondary/50 hover:bg-secondary/50">
                      <TableHead className="md:w-[30%] md:px-4 uppercase tracking-wider text-xs font-semibold">
                        Nome
                      </TableHead>
                      <TableHead className="md:w-[16%] md:px-4 uppercase tracking-wider text-xs font-semibold">
                        Criado em
                      </TableHead>
                      <TableHead className="md:w-[16%] md:px-4 uppercase tracking-wider text-xs font-semibold">
                        Valor do serviço
                      ...
                      <TableHead className="md:w-[10%] md:px-4 uppercase tracking-wider text-xs font-semibold">
                        Margem liq.
                      </TableHead>
                      <TableHead className="md:w-[12%] md:px-4 text-right uppercase tracking-wider text-xs font-semibold">
                        Ações
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {rows.map((r) => {
                      ...
                      const createdFmt = created.toLocaleString("pt-BR", {
                        dateStyle: "short",
                        timeStyle: "short",
                      });
                      return (
                        <TableRow
                          key={r.id}
                          className="cursor-pointer hover:bg-secondary/30 border-border/40"
                        ...
                        >
                          ...
                          <TableCell className="md:p-4 align-top whitespace-normal text-sm">
                            {createdFmt}
                          </TableCell>
                          <TableCell className="md:p-4 align-top break-all whitespace-normal text-right text-sm">
                            {formatBRL(r.serviceValue)}
                          </TableCell>
                          <TableCell className="md:p-4 align-top break-all whitespace-normal text-right text-sm">
                          ...
                          <TableCell className="md:p-4 align-top text-right">
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded-md border text-xs font-semibold ${getMarginBadgeClasses(r.netMargin ?? r.actualMargin)}`}
                            >
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      A simulação "{r.name}" será excluída
                                      permanentemente.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
