...
describe("tax calculations", () => {
  ...
  it("MEI tax rate (5%)", () => {
    const r = calculateMargin(
      input({ taxRate: 5, serviceValue: 1000, variableCost: 500 })
    );
    expect(r.taxAmount).toBe(50);
  ...
  it("Simples Nacional Anexo III (6%)", () => {
    const r = calculateMargin(
      input({ taxRate: 6, serviceValue: 1000, variableCost: 500 })
    );
  ...
  it("Simples Nacional Anexo V (15.5%)", () => {
    const r = calculateMargin(
      input({ taxRate: 15.5, serviceValue: 1000, variableCost: 500 })
    );
  ...
  it("Lucro Presumido (16.33%)", () => {
    const r = calculateMargin(
      input({ taxRate: 16.33, serviceValue: 1000, variableCost: 500 })
    );
...
describe("fixed cost calculations", () => {
  it("computes fixed cost per job", () => {
    const r = calculateMargin(
      input({ fixedCostMonthly: 2000, jobsPerMonth: 20 })
    );
  ...
  it("adds fixed cost per job to total cost", () => {
    const r = calculateMargin(
      input({ variableCost: 400, fixedCostMonthly: 2000, jobsPerMonth: 20 })
    );
    // 400 + (2000/20) = 500
  ...
  it("reduces gross margin when fixed costs are included", () => {
    const noFixed = calculateMargin(
      input({ variableCost: 400, fixedCostMonthly: 0 })
    );
    const withFixed = calculateMargin(
      input({ variableCost: 400, fixedCostMonthly: 2000, jobsPerMonth: 20 })
    );
  ...
  it("minimum price increases when fixed costs are included", () => {
    const noFixed = calculateMargin(
      input({ desiredMargin: 30, fixedCostMonthly: 0 })
    );
    const withFixed = calculateMargin(
      input({ desiredMargin: 30, fixedCostMonthly: 2000, jobsPerMonth: 20 })
    );
  ...
  it("uses jobsPerMonth=1 as floor when 0 is provided", () => {
    const r = calculateMargin(
      input({ fixedCostMonthly: 1000, jobsPerMonth: 0 })
    );
  ...
  it("uses jobsPerMonth=1 as floor for negative values", () => {
    const r = calculateMargin(
      input({ fixedCostMonthly: 1000, jobsPerMonth: -5 })
    );
    expect(r.fixedPerJob).toBe(1000);
  });
});
...
describe("status relative to desired margin", () => {
  it('returns "safe" when net margin >= desired margin', () => {
    ...
    const r = calculateMargin(
      input({ serviceValue: 1000, variableCost: 400, desiredMargin: 30 })
    );
    expect(r.status).toBe("safe");
  ...
  it('returns "warning" when net margin >= 0 but below desired margin', () => {
    ...
    const r = calculateMargin(
      input({ serviceValue: 1000, variableCost: 800, desiredMargin: 30 })
    );
    expect(r.status).toBe("warning");
  ...
  it('returns "danger" when net margin < 0 with desired margin set', () => {
    ...
    const r = calculateMargin(
      input({ serviceValue: 100, variableCost: 200, desiredMargin: 30 })
    );
    expect(r.status).toBe("danger");
  ...
  it('returns "safe" at exactly desired margin', () => {
    ...
    const r = calculateMargin(
      input({ serviceValue: 1000, variableCost: 700, desiredMargin: 30 })
    );
    expect(r.status).toBe("safe");
  });
});
...
describe("edge cases", () => {
  ...
  it("zero variable cost → margin is 100% minus tax", () => {
    const r = calculateMargin(
      input({ serviceValue: 1000, variableCost: 0, taxRate: 10 })
    );
  ...
  it("very large service value does not overflow", () => {
    const r = calculateMargin(
      input({ serviceValue: 1_000_000_000, variableCost: 500_000_000 })
    );
    expect(Number.isFinite(r.grossMargin)).toBe(true);
  ...
  it("very small fractional values work", () => {
    const r = calculateMargin(
      input({ serviceValue: 0.01, variableCost: 0.005, taxRate: 0 })
    );
    expect(round(r.grossMargin)).toBe(50);
  });
});
...
describe("max discount", () => {
  it("is 0 when service value equals minimum price", () => {
    ...
    const r = calculateMargin(
      input({ serviceValue: minPrice, variableCost: 400, desiredMargin: 30 })
    );
  ...
  it("is clamped to 0 when service value is below minimum price", () => {
    const r = calculateMargin(
      input({ serviceValue: 100, variableCost: 400, desiredMargin: 30 })
    );
  ...
  it("increases when service value is well above minimum price", () => {
    const r = calculateMargin(
      input({ serviceValue: 2000, variableCost: 400, desiredMargin: 30 })
    );
