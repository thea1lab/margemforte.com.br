...
export function importCalculations(json: string): number {
  const incoming: SavedCalculation[] = JSON.parse(json);
  if (!Array.isArray(incoming))
    throw new Error("Formato inválido: esperava uma lista de simulações.");
  const existing = readAll();
  const existingIds = new Set(existing.map((c) => c.id));
  const newItems = incoming.filter((c) => c.id && !existingIds.has(c.id));
  if (newItems.length > 0) {
    writeAll([...newItems, ...existing]);
  }
...
export function exportCalculationsCSV(): string {
  ...
  const header =
    "Nome,Template,Valor do Serviço,Custo Total,Margem Desejada,Margem Bruta,Margem Líquida,Impostos (%),Impostos (R$),Custo Fixo/Serviço,Preço Mínimo,Desconto Máximo,Criado em";
