...
export const MarginCalculator = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      if (data) {
        ...
        setFixedCostMonthly(
          data.fixedCostMonthly
            ? formatNumberToInput(data.fixedCostMonthly)
            : ""
        );
      ...
      prev.map((item) => {
        ...
        if (field === "value") {
          if (item.type === "currency")
            return { ...item, value: formatCurrency(value) };
          return { ...item, value };
        }
  ...
  useEffect(() => {
    if (parseCurrency(serviceValue) > 0) {
      const r = calcMargin({
        ...
        variableCost: costItems.reduce(
          (sum, item) => sum + costItemValue(item, parseCurrency),
          0
        ),
      ...
      setResult({ ...r, actualMargin: r.grossMargin });
    } else {
      setResult(null);
    }
  }, [
      ...
      <div className="space-y-8">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[hsl(235,55%,20%)] via-[hsl(245,50%,30%)] to-[hsl(255,45%,35%)] px-6 py-12 sm:px-10 sm:py-16 md:py-20 text-center">
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/70 mt-4 text-base sm:text-lg max-w-2xl mx-auto">
              Descubra a margem real do seu negócio - com custos fixos,
        ...
        <section className="mb-6">
          <p className="text-sm text-muted-foreground/80 leading-relaxed max-w-3xl">
            Calcule a margem de lucro real do seu negócio de forma rápida e
            gratuita. Inclua custos fixos, variáveis e impostos (MEI, Simples
            Nacional, Lucro Presumido) para descobrir o preço mínimo de venda, o
            desconto máximo e o lucro líquido por serviço. Ideal para
            prestadores de serviço, confeiteiros, artesãos, freelancers e
            consultores.
          </p>
        </section>
  ...
  const variableCostNumber = costItems.reduce(
    (sum, item) => sum + costItemValue(item, parseCurrency),
    0
  );
  ...
  const minimumPriceDisplay =
    result && isFinite(result.minimumPrice)
      ? formatBRL(result.minimumPrice)
      : "";
  const minPriceTextClass = minimumPriceDisplay
    ? getNumberSizeClasses(minimumPriceDisplay)
    : "text-3xl sm:text-4xl md:text-5xl";
  const netMarginDisplay = result ? `${result.netMargin.toFixed(2)}%` : "";
  const netMarginTextClass = netMarginDisplay
    ? getNumberSizeClasses(netMarginDisplay)
    : "text-3xl sm:text-4xl md:text-5xl";
  const maxDiscountDisplay =
    result && isFinite(result.maxDiscount)
      ? `${result.maxDiscount.toFixed(2)}%`
      : "";
  const maxDiscountTextClass = maxDiscountDisplay
    ? getNumberSizeClasses(maxDiscountDisplay)
    : "text-3xl sm:text-4xl md:text-5xl";

  const netProfitValue = result
    ? serviceValueNumber - result.totalCost - result.taxAmount
    : 0;
  const netProfitDisplay = result ? formatBRL(Math.max(0, netProfitValue)) : "";
  const netProfitTextClass = netProfitDisplay
    ? getSmallNumberSizeClasses(netProfitDisplay)
    : "text-base md:text-lg";
  const taxAmountDisplay = result ? formatBRL(result.taxAmount) : "";
  const taxAmountTextClass = taxAmountDisplay
    ? getSmallNumberSizeClasses(taxAmountDisplay)
    : "text-base md:text-lg";
  const totalCostDisplay = result ? formatBRL(result.totalCost) : "";
  const totalCostTextClass = totalCostDisplay
    ? getSmallNumberSizeClasses(totalCostDisplay)
    : "text-base md:text-lg";
  const serviceValueDisplay = result ? formatBRL(serviceValueNumber) : "";
  const serviceValueTextClass = serviceValueDisplay
    ? getSmallNumberSizeClasses(serviceValueDisplay)
    : "text-base md:text-lg";
    ...
    <div className="space-y-8 mx-auto scroll-smooth">
      ...
      <Card className="border-0 rounded-2xl shadow-medium overflow-hidden">
        ...
        <CardContent className="px-5 sm:px-8 pb-8 space-y-8">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <TooltipProvider delayDuration={100}>
                    <Tooltip>
                      ...
                      <TooltipContent side="top" className="max-w-xs text-left">
                        Percentual de lucro-alvo. Usada para calcular o preço
                        mínimo e o desconto máximo. Não altera a sua margem
                        atual.
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
          ...
          <div className="p-5 sm:p-6 bg-secondary/50 rounded-xl">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
              <div className="flex-1 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <TooltipProvider delayDuration={100}>
                    <Tooltip>
                      ...
                      <TooltipContent side="top" className="max-w-xs text-left">
                        Percentual de impostos sobre o valor do serviço.
                        Consulte seu contador para a alíquota exata.
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
                <div className="flex flex-wrap gap-2">
                  {TAX_REGIMES.filter((r) => r.id !== "custom").map(
                    (regime) => (
                ...
                <p className="text-xs text-muted-foreground">
                  Selecione seu regime tributário ou informe a alíquota
                  manualmente.
                </p>
              </div>
              <div className="text-center sm:text-right">
          ...
          <div className="p-5 sm:p-6 bg-secondary/50 rounded-xl space-y-4">
            <div className="flex items-center gap-2">
              ...
              <TooltipProvider delayDuration={100}>
                <Tooltip>
                  ...
                  <TooltipContent side="top" className="max-w-xs text-left">
                    Custos que você paga todo mês independente de quantos
                    serviços faz. Ex: aluguel, energia, internet, contador.
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-1">
                  <span className="text-xs text-muted-foreground">
                    Quantos serviços/vendas por mês?
                  </span>
                  <TooltipProvider delayDuration={100}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                      ...
                      <TooltipContent side="top" className="max-w-xs text-left">
                        Quantos serviços ou vendas você faz por mês, em média.
                        Usado para dividir os custos fixos por cada serviço.
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
            ...
            <p className="text-xs text-muted-foreground">
              {selectedTemplate.suggestedFixedCosts &&
              selectedTemplate.suggestedFixedCosts.length > 0
          ...
          <div className="flex gap-3 p-4 rounded-xl bg-blue-50 border border-blue-200 text-blue-800">
            ...
            <p className="text-sm">
              Para calcular sua margem real, preencha <strong>todos</strong> os
              campos: o preço cobrado, os custos de cada serviço, seus gastos
              fixos do mês e os impostos.
            </p>
          </div>
          ...
          <div className="space-y-3">
            <div>
              <Label className="text-sm font-medium">
                Custos por serviço (variáveis)
              </Label>
              <p className="text-xs text-muted-foreground mt-0.5">
                Quanto você gasta para fazer <strong>cada</strong> serviço ou
                produto.
              </p>
            </div>
            {costItems.map((item) => (
              <div
                key={item.id}
                className="p-4 sm:p-5 bg-secondary/30 rounded-xl border border-border/50"
              >
                <div className="flex items-start gap-3">
                  <div className="flex-1 space-y-2">
                    <Input
                      value={item.label}
                      onChange={(e) =>
                        handleCostItemChange(item.id, "label", e.target.value)
                      }
            ...
            {showAddItem ? (
              <div className="p-4 sm:p-5 bg-secondary/30 rounded-xl border border-border/50 space-y-2">
                ...
                <Select
                  value={newItemType}
                  onValueChange={(v) => setNewItemType(v as CostItemType)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    <SelectItem value="quantity_price">
                      Qtd x Preço unit.
                    </SelectItem>
                  </SelectContent>
                </Select>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              {fixedPerJobNumber > 0 && (
                <div className="space-y-0.5">
                  ...
                  <span className="text-sm font-medium block">
                    {renderBreakable(formatBRL(fixedPerJobNumber))}
                  </span>
                </div>
              )}
              <div className="space-y-0.5 col-span-2 md:col-span-1">
                ...
                <span className="text-sm font-bold block">
                  {renderBreakable(formatBRL(totalCostNumber))}
                </span>
              </div>
            </div>
          </div>
      ...
      {result && (
        <Card className="border-0 rounded-2xl shadow-medium overflow-hidden animate-fade-in">
          {/* Status banner */}
          <div
            className={`px-5 sm:px-8 py-4 flex items-center gap-3 border-b ${getStatusBg(result.status)}`}
          >
          ...
          <CardContent className="px-5 sm:px-8 py-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="p-6 rounded-xl bg-secondary/40 text-center">
                ...
                <div
                  className={`${minPriceTextClass} font-extrabold text-primary mt-2 break-words`}
                >
                  {isFinite(result.minimumPrice)
                    ? renderBreakable(minimumPriceDisplay)
                    : "indefinido"}
                </div>
                <span className="text-xs text-muted-foreground mt-1 block">
                  com margem de {desiredMargin}%
                  {currentTaxRateNum > 0
                    ? ` + ${currentTaxRateNum}% impostos`
                    : ""}
                </span>
              </div>
              ...
              {/* Margem líquida */}
              <div
                className={`p-6 rounded-xl text-center ${getMarginBg(result.status)}`}
              >
                <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  Margem líquida
                </span>
                <div
                  className={`${netMarginTextClass} font-extrabold mt-2 break-words ${getMarginColor(result.status)}`}
                >
              ...
              <div className="p-6 rounded-xl bg-secondary/40 text-center">
                ...
                <div
                  className={`${maxDiscountTextClass} font-extrabold text-primary mt-2 break-words`}
                >
                  {isFinite(result.maxDiscount)
                    ? renderBreakable(maxDiscountDisplay)
                    : "indefinido"}
                </div>
                <span className="text-xs text-muted-foreground mt-1 block">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-4 rounded-xl bg-secondary/20">
                ...
                <span
                  className={`${netProfitTextClass} font-bold ${getMarginColor(result.status)} break-words`}
                >
              ...
              <div className="text-center p-4 rounded-xl bg-secondary/20">
                ...
                {result.fixedPerJob > 0 && (
                  <span className="text-xs text-muted-foreground block">
                    var. {formatBRL(result.variableCost)} + fixo{" "}
                    {formatBRL(result.fixedPerJob)}
                  </span>
                )}
              </div>
              <div className="text-center p-4 rounded-xl bg-secondary/20">
                ...
                <span
                  className={`${serviceValueTextClass} font-bold break-words`}
                >
            ...
            <Accordion
              type="single"
              collapsible
              className="p-4 border rounded-xl bg-secondary/20"
            >
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      <p className="text-muted-foreground">
                        1) Custo total (variável + fixo)
                      </p>
                      <p className="mb-1">
                        Objetivo: somar todos os custos (sem lucro).
                      </p>
                      ...
                      <div className="font-mono text-xs space-y-0.5">
                        {costItems.map((item) => (
                          <div key={item.id}>
                            {item.label}:{" "}
                            {formatBRL(costItemValue(item, parseCurrency))}
                          </div>
                        ))}
                        {result.fixedPerJob > 0 && (
                          <div>
                            Custo fixo/serviço: {formatBRL(result.fixedPerJob)}{" "}
                            ({formatBRL(fixedMonthlyNumber)} / {jobsNumber}{" "}
                            serviços)
                          </div>
                        )}
                      </div>
                    ...
                    <div>
                      <p className="text-muted-foreground">
                        2) Preço mínimo com margem desejada
                        {currentTaxRateNum > 0 ? " e impostos" : ""}
                      </p>
                      <p className="mb-1">
                        Objetivo: descobrir o menor preço para atingir a margem
                        desejada.
                      </p>
                      <div className="overflow-x-auto">
                        <BlockMath>
                      ...
                      <p className="font-mono">
                        {formatBRL(result.totalCost)} / (1 -{" "}
                        {Number(desiredMargin) || 0}%
                        {currentTaxRateNum > 0
                          ? ` - ${currentTaxRateNum}%`
                          : ""}
                        ) ={" "}
                    ...
                    <div>
                      ...
                      <p className="mb-1">
                        Lucro antes dos impostos, em relação ao preço.
                      </p>
                      <div className="overflow-x-auto">
                        <BlockMath>{String.raw`\%\,\text{Margem bruta} = \dfrac{\text{Valor} - \text{Custo total}}{\text{Valor}} \times 100`}</BlockMath>
                      </div>
                      <p className="font-mono">
                        ({formatBRL(serviceValueNumber)} -{" "}
                        {formatBRL(result.totalCost)}) /{" "}
                        {formatBRL(serviceValueNumber)} x 100 ={" "}
                        <span className="font-bold">
                    ...
                    <div>
                      ...
                      <p className="mb-1">
                        Valor dos impostos sobre o preço do serviço.
                      </p>
                      <div className="overflow-x-auto">
                        <BlockMath>{String.raw`\text{Impostos} = \text{Valor} \times \text{Alíquota}`}</BlockMath>
                      </div>
                      <p className="font-mono">
                        {formatBRL(serviceValueNumber)} x {currentTaxRateNum}% ={" "}
                        <span className="font-bold">
                          {formatBRL(result.taxAmount)}
                        </span>
                      </p>
                    </div>
                    <Separator />
                    <div>
                      ...
                      <p className="mb-1">
                        O que sobra de fato após custos e impostos.
                      </p>
                      <div className="overflow-x-auto">
                    ...
                    <div>
                      ...
                      <p className="mb-1">
                        Objetivo: saber quanto desconto ainda cabe mantendo a
                        margem desejada.
                      </p>
                      <div className="overflow-x-auto">
            ...
            <div className="max-w-md mx-auto w-full">
              <Label htmlFor="simulationName" className="text-sm">
                Nome da simulação <span className="text-destructive">*</span>
              </Label>
              <Input
                id="simulationName"
      ...
      {!result && (
        <Card className="border-0 rounded-2xl shadow-soft p-6 sm:p-8 text-center">
          <div className="flex flex-col items-center gap-2 text-muted-foreground">
            ...
            <p className="text-sm">
              Preencha os campos acima para ver o resultado do cálculo.
            </p>
            <p className="text-xs">
              Informe ao menos o Valor do Serviço para começarmos.
            </p>
          </div>
        </Card>
      )}
